...
这对 Knead 很有价值，因为我们不是要凭空发明一套复杂信息结构，而是先承认：如果一个 AI 产品要好用，它首先必须像一个成熟 AI 助手一样好用。
...
LobeHub Agent Config
  -> 是 Knead Product Manifest 的一部分

LobeHub Artifact
  -> 是 Knead Product UI Runtime 的雏形

LobeHub Agent Builder
  -> 是 Knead Product Builder 的低阶版本
```
...
这对我们很重要。Knead 首页也不应该只是“一个空输入框”，而应该是：
...
LobeHub 证明了“长期 AI 助手对象”是合理的；Knead 要做的是把这个对象继续升级成“长期 AI 产品对象”。
