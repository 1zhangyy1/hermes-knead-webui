...
def _local_static_suffix() -> str:
    """Cache-bust local WebUI hotfixes when the release version is unchanged."""
    ...
    try:
        ...
        return ""
    return f"-local-{max(mtimes)}" if mtimes else ""
...
def _detect_webui_version() -> str:
    ...
    if out:
        return out + _local_static_suffix()
    ...
    if version_file.exists():
        try:
            ...
            if m:
                return m.group(1) + _local_static_suffix()
        except Exception:
            pass

    return 'unknown' + _local_static_suffix()
