...
async function cmdQueue(args){
  ...
  if(!S.session){showToast(t('no_active_session'));return;}
  const payload={text:msg,files:[...S.pendingFiles],model:S.session&&S.session.model||($('modelSelect')&&$('modelSelect').value)||'',model_provider:S.session&&S.session.model_provider||null,profile:S.activeProfile||'default'};
  queueSessionMessage(S.session.session_id,typeof withCurrentProductContext==='function'?withCurrentProductContext(payload,msg):payload);
  updateQueueBadge(S.session.session_id);
...
async function _trySteer(msg, explicitSteer){
  let result=null;
  try{
    const steerPayload = typeof withCurrentProductContext === 'function'
      ? withCurrentProductContext({session_id:S.session.session_id,text:msg}, msg, {consumePending:false})
      : {session_id:S.session.session_id,text:msg};
