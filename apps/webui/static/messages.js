...
function _productUiStatusContext(productContext = {}) {
  const productId = String(productContext.product_id || productContext.productId || '').trim();
  const productTitle = String(
    productContext.product_title ||
    productContext.productTitle ||
    window._currentAiProductTitle ||
    window._currentAiAssistantTitle ||
    '这个 AI 产品'
  ).trim();
  const productScope = String(productContext.product_scope || productContext.productScope || '').trim();
  return {productId, productTitle, productScope};
}

function _applyProductContextToCurrentSession(productContext = {}) {
  if (!productContext || !productContext.product_id || typeof S === 'undefined' || !S.session) return false;
  S.session.product_id = productContext.product_id;
  S.session.product_scope = productContext.product_scope || S.session.product_scope || 'product_usage';
  S.session.product_intent = productContext.product_intent || S.session.product_intent || '';
  return true;
}
...
function _productUiStatusCard(phase, context = {}, options = {}) {
  ...
  const actions = [];
  if (phase === 'failed') actions.push({id:'product-regenerate', label:'重新生成界面'});
  if (!isInit && phase === 'done' && options.canRollback) actions.push({id:'product-rollback', label:'恢复上一版'});
  return {
    title: phase === 'running'
      ? (isInit ? '正在生成这个 AI 产品的第一版界面' : '正在改这个 AI 产品的界面')
      : phase === 'failed'
        ? (isInit ? '第一版产品界面生成失败' : '产品界面改造失败')
        : (isInit ? '第一版产品界面已生成' : '已生成新版产品界面'),
    subtitle: phase === 'running'
      ? (isInit ? '继续在这里聊天，右侧会自动出现这个产品的界面。' : '继续在这里聊天，右侧产品界面会更新。')
      : phase === 'failed'
        ? (failureReason
          ? `${failureReason} 可以重新生成界面，或继续说明你想让这个产品怎么工作。`
          : (isInit ? '可以重新生成界面，或继续说明你想让这个产品怎么工作。' : '可以重新生成界面，或直接说想让这个界面怎么改。'))
        : (isInit ? '右侧已刷新，可以继续用普通聊天让它完成任务或调整界面。' : (options.canRollback ? '右侧已刷新，可恢复上一版。' : '右侧已刷新，可以继续聊天调整。')),
    rows,
    actions
  };
}

function _removeProductUiStatusCards(productId = '') {
  if (!Array.isArray(S.messages)) return;
  const id = String(productId || '').trim();
  S.messages = S.messages.filter(message => {
    if (!message || !message._productUiStatus) return true;
    if (!id) return false;
    return String(message._productUiStatus.productId || '') !== id;
  });
}

function _appendProductUiStatusCard(phase, context = {}, options = {}) {
  if (!Array.isArray(S.messages)) return false;
  const productId = String(context.productId || '').trim();
  _removeProductUiStatusCards(productId);
  S.messages.push({
    role:'assistant',
    content:'',
    _ts:Date.now()/1000,
    _transient:true,
    _productUiStatus:{phase, productId},
    _statusCard:_productUiStatusCard(phase, context, options)
  });
...
function _productFailureInfoFromAppError(payload = {}, label = '') {
  ...
  if (type === 'no_response' || type === 'silent_failure') {
    return {
      type: 'no_response',
      message: '模型这次没有返回内容，可能是 provider 超时或限流。'
    };
  }
  if (type === 'model_not_found') {
    return {type, message: '当前模型不可用，请检查模型设置后重试。'};
  }
  if (type === 'auth_mismatch') {
    return {type, message: '当前 provider 或凭据不可用，请检查模型配置后重试。'};
  }
  if (type === 'rate_limit' || type === 'quota_exhausted') {
...
async function _markProductUiFailed(context = {}, reason = '', failure = {}) {
  const productScope = String(context && context.productScope || '').trim();
  if (!context || !['product_init', 'product_builder'].includes(productScope)) return false;
  const productId = String(context.productId || '').trim();
  if (!productId) return false;
  try {
    if (typeof _setProductUiStatus === 'function') {
      const kind = typeof _assistantKindForProductId === 'function' ? _assistantKindForProductId(productId) : '';
      const object = kind && typeof _assistantObject === 'function'
        ? _assistantObject(kind)
        : {productId, product_id: productId};
      await _setProductUiStatus(object, 'failed', {
        persist: true,
      ...
      await api('/api/products/update', {
        ...
        body: JSON.stringify({
          ...
          ui_status: 'failed',
          ui_error_type: failure.type || '',
          ui_error_message: failure.message || ''
        })
      });
    }
    if (typeof refreshCurrentProductPreview === 'function') {
      await refreshCurrentProductPreview({silent: true, reason: reason || 'product-ui-failed'});
    }
    return true;
  } catch (err) {
    console.debug('Failed to mark product UI as failed', err);
    return false;
  }
}
...
async function send(){
  ...
  if (_sendInProgress) {
    ...
    const _queueText=_text;
    if(_text && S.session && S.session.session_id){
      const _queuePayload={text:_queueText,sendText:_text,files:[...S.pendingFiles],model:S.session&&S.session.model||($('modelSelect')&&$('modelSelect').value)||'',model_provider:S.session&&S.session.model_provider||null,profile:S.activeProfile||'default'};
      queueSessionMessage(S.session.session_id,typeof withCurrentProductContext==='function'?withCurrentProductContext(_queuePayload,_text):_queuePayload);
      $('msg').value='';autoResize();
      S.pendingFiles=[];renderTray();
      updateQueueBadge(S.session.session_id);
      showToast(`Queued: "${_queueText.slice(0,40)}${_queueText.length>40?'…':''}"`,2000);
    }
    return;
  }
  ...
  if(S.busy||compressionRunning){
    if(text){
      ...
      const queueDisplayText=text;
      const queuePayloadBase=typeof withCurrentProductContext==='function'?withCurrentProductContext({
        text:queueDisplayText,
        sendText:text,
        files:[...S.pendingFiles],
        model:S.session&&S.session.model||($('modelSelect')&&$('modelSelect').value)||'',
      ...
      if(busyMode==='steer'&&S.activeStreamId&&typeof _trySteer==='function'){
        ...
        queueSessionMessage(S.session.session_id,queuePayloadBase);
        updateQueueBadge(S.session.session_id);
        $('msg').value='';autoResize();
        S.pendingFiles=[];renderTray();
        if(S.activeStreamId&&typeof cancelStream==='function'){
          ...
          showToast(`Queued: "${queueDisplayText.slice(0,40)}${queueDisplayText.length>40?'…':''}"`,2000);
        }
      } else {
  ...
  if(!S.session||!S.session.session_id){
    setComposerStatus('Error: could not create a task session');
    if(typeof showToast==='function') showToast('无法创建任务会话，请稍后重试',3000);
    return;
  }
  if (typeof markSessionForCurrentAssistant === 'function') markSessionForCurrentAssistant(S.session.session_id, text);
  ...
  const productContext = typeof currentAssistantProductContextForMessage === 'function'
    ...
    : null;
  _applyProductContextToCurrentSession(productContext);
  const productUiStatusContext = productContext && (productContext.product_scope === 'product_init' || productContext.product_scope === 'product_builder')
    ? _productUiStatusContext(productContext)
    : null;
  ...
  const titleCandidate=text||(uploaded.length?`Uploaded: ${uploadedNames.join(', ')}`:'(file upload)');
  const displayText=text||titleCandidate;
  ...
  if(productUiStatusContext){
    _appendProductUiStatusCard('running', productUiStatusContext, {render:false});
  }
  ...
  if(typeof upsertActiveSessionForLocalTurn==='function'){
    upsertActiveSessionForLocalTurn({title:titleCandidate.slice(0,64),messageCount:S.messages.length,timestampMs:Date.now()});
  }
  INFLIGHT[activeSid]={messages:[...S.messages],uploaded:uploadedNames,toolCalls:[],productUiStatusContext};
  if(typeof saveInflightState==='function'){
    saveInflightState(activeSid,{streamId:null,messages:INFLIGHT[activeSid].messages,uploaded:uploadedNames,toolCalls:[],productUiStatusContext});
  }
  if(typeof renderSessionListFromCache==='function') renderSessionListFromCache();
  ...
  if(S.session&&(S.session.title==='Untitled'||!S.session.title)){
    const provisionalTitle=titleCandidate.slice(0,64);
    ...
    upsertActiveSessionForLocalTurn({title:S.session&&S.session.title||titleCandidate.slice(0,64),messageCount:S.messages.length,timestampMs:Date.now()});
  } else {
  ...
  try{
    ...
    if(typeof saveInflightState==='function'){
      saveInflightState(activeSid,{streamId,messages:INFLIGHT[activeSid].messages,uploaded:uploadedNames,toolCalls:INFLIGHT[activeSid].toolCalls||[],productUiStatusContext:INFLIGHT[activeSid].productUiStatusContext||null});
    }
...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function persistInflightState(){
    ...
    saveInflightState(activeSid,{
      ...
      productUiStatusContext:inflight.productUiStatusContext||null,
    });
  }
  function snapshotLiveTurn(){
    if(typeof snapshotLiveTurnHtmlForSession==='function') snapshotLiveTurnHtmlForSession(activeSid);
  }
  function _inflightProductUiStatusContext(){
    return INFLIGHT[activeSid]&&INFLIGHT[activeSid].productUiStatusContext
      ? INFLIGHT[activeSid].productUiStatusContext
      : null;
  }
  ...
  function _liveThinkingText(){
    ...
    return clean || (typeof _thinkingPlaceholderText==='function' ? _thinkingPlaceholderText() : 'Thinking…');
  }
  ...
  function _renderLiveThinking(parsed){
    ...
    if(text||(parsed&&parsed.inThinking)){
      if(typeof updateThinking==='function') updateThinking(text||(typeof _thinkingPlaceholderText==='function'?_thinkingPlaceholderText():'Thinking…'));
      else appendThinking();
      return;
    }
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('done',e=>{
      ...
      const _finishDone=()=>{
        ...
        const productUiStatusContext = INFLIGHT[activeSid] && INFLIGHT[activeSid].productUiStatusContext
          ? INFLIGHT[activeSid].productUiStatusContext
          : null;
        ...
        if(isActiveSession&&_pendingGoalContinuation&&typeof queueSessionMessage==='function'){
          ...
          queueSessionMessage(_goalNext.sid,typeof withCurrentProductContext==='function'?withCurrentProductContext(_goalPayload,_goalNext.text,{consumePending:false}):_goalPayload);
    ...
    source.addEventListener('pending_steer_leftover',e=>{
      ...
      try{
        ...
        if(typeof queueSessionMessage==='function'){
          ...
          queueSessionMessage(sid,typeof withCurrentProductContext==='function'?withCurrentProductContext(leftoverPayload,txt,{consumePending:false}):leftoverPayload);
    ...
    source.addEventListener('apperror',e=>{
      ...
      source.close();
      const productUiErrorContext=_inflightProductUiStatusContext();
      let productUiFailureInfo=null;
      _clearOwnerInflightState();
      _clearApprovalForOwner();
      _clearClarifyForOwner('terminal');
      if(S.session&&S.session.session_id===activeSid){
        ...
        try{
          ...
          productUiFailureInfo=_productFailureInfoFromAppError(d,label);
          const hint=d.hint?`\n\n*${d.hint}*`:'';
        ...
        if(productUiErrorContext){
          _appendProductUiStatusCard('failed', productUiErrorContext, {
            render:false,
            failureReason:productUiFailureInfo&&productUiFailureInfo.message||''
          });
          void _markProductUiFailed(productUiErrorContext, 'product-ui-app-error', productUiFailureInfo||{});
        }
    ...
    source.addEventListener('cancel',e=>{
      ...
      if(typeof finalizeThinkingCard==='function') finalizeThinkingCard();
      source.close();
      const productUiCancelContext=_inflightProductUiStatusContext();
      _clearOwnerInflightState();
      _clearApprovalForOwner();
      ...
      (async()=>{
        try{
          ...
          if(data&&data.session&&S.session&&S.session.session_id===activeSid){
            ...
            clearLiveToolCards();if(!assistantText)removeThinking();
            if(productUiCancelContext){
              _appendProductUiStatusCard('failed', productUiCancelContext, {render:false});
              void _markProductUiFailed(productUiCancelContext, 'product-ui-cancelled');
            }
  ...
  function _handleStreamError(){
    ...
    if(S.session&&S.session.session_id===activeSid){
      ...
      if(productUiErrorContext){
        _appendProductUiStatusCard('failed', productUiErrorContext, {render:false});
        void _markProductUiFailed(productUiErrorContext, 'product-ui-connection-lost');
      }
      renderMessages({preserveScroll:true});
      _markSessionViewed(activeSid, S.messages.length);
    }else{
