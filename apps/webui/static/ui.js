...
function handleStatusCardAction(btn){
  const action=String(btn&&btn.dataset&&btn.dataset.statusCardAction||'').trim();
  if(!action)return;
  if(action==='product-regenerate'&&typeof requestCurrentProductUiGeneration==='function'){
...
function _statusCardHtml(card){
  ...
  const actionHtml=actions.length?`<div class="status-card-actions">${actions.map(action=>{
    const id=String(action&&action.id||'').trim();
    const label=String(action&&action.label||'').trim();
    if(!id||!label)return '';
    return `<button type="button" data-status-card-action="${esc(id)}" onclick="handleStatusCardAction(this);event.stopPropagation()">${esc(label)}</button>`;
  }).join('')}</div>`:'';
...
function _questionJumpButtonHtml(questionRawIdx){
  ...
  const label=(typeof _isNextAiProductView==='function'&&_isNextAiProductView()) ? '回到问题' : (t('jump_to_question')||'Question');
  const title=(typeof _isNextAiProductView==='function'&&_isNextAiProductView()) ? '跳到这条回答对应的问题' : (t('jump_to_question_label')||'Jump to the question for this response');
...
function setBusy(v){
  ...
  if(!v){
    ...
    if(next){
      updateQueueBadge(sid);
      setTimeout(()=>{
        $('msg').value=next.sendText||next.text||'';
        if(next.product_scope||next.productScope){
          window._nextAiPendingProductScope=String(next.product_scope||next.productScope||'').trim();
        }
  ...
  if(q.length>1){
    ...
    mergeBtn.onclick=()=>{
      const _doMerge=(snapshot)=>{
        ...
        liveQ.length=0;liveQ.push({
          text:combined,
          files:firstFiles,
          model:first.model||'',
          model_provider:first.model_provider||null,
          product_id:first.product_id||first.productId||'',
          product_scope:first.product_scope||first.productScope||'',
          product_intent:first.product_intent||first.productIntent||combined,
          _queued_at:Date.now()
        });
...
function syncTopbar(){
  if(typeof syncAssistantTaskUi==='function') syncAssistantTaskUi();
  const assistantChrome = typeof assistantChromeTitleState === 'function'
  ...
  if(_topbarMeta){
    ...
    const metaText=assistantChrome&&assistantChrome.sub
      ? `${assistantChrome.sub} · ${t('n_messages',vis.length)}`
      : t('n_messages',vis.length);
...
function _assistantRoleHtml(tsTitle='', tpsText=''){
  const _bn=window._currentAiAssistantTitle||window._botName||'Hermes';
...
function _isNextAiProductView(){
  return !!(document.body && document.body.dataset && document.body.dataset.nextAiView);
}
function _thinkingLabelText(){
  return _isNextAiProductView() ? '思考中' : t('thinking');
}
function _thinkingPlaceholderText(){
  return _isNextAiProductView() ? '思考中…' : (t('voice_thinking') || 'Thinking…');
}
function _thinkingCardHtml(text, open){
  ...
  return `<div class="${classes}"><div class="thinking-card-header" onclick="this.parentElement.classList.toggle('open')"><span class="thinking-card-icon">${li('lightbulb',14)}</span><span class="thinking-card-label">${esc(_thinkingLabelText())}</span><span class="thinking-card-btn-row">${copyBtn}<span class="thinking-card-toggle">${li('chevron-right',12)}</span></span></div><div class="thinking-card-body"><pre>${esc(clean)}</pre></div></div>`;
}
...
function renderMessages(options){
  ...
  for(let vi=0;vi<renderVisWithIdx.length;vi++){
    ...
    const displayContent=isUser
      ? _stripWorkspaceDisplayPrefix(content)
      : content;
...
function _thinkingMarkup(text=''){
  ...
  return (clean&&String(clean).trim())
    ? `<div class="thinking-card${openClass}"><div class="thinking-card-header" onclick="this.parentElement.classList.toggle('open')"><span class="thinking-card-icon">${li('lightbulb',14)}</span><span class="thinking-card-label">${esc(_thinkingLabelText())}</span><span class="thinking-card-toggle">${li('chevron-right',12)}</span></div><div class="thinking-card-body"><pre>${esc(String(clean).trim())}</pre></div></div>`
...
function appendThinking(text='', options){
  ...
  const thinkingText=String(text||'').trim()||_thinkingPlaceholderText();
