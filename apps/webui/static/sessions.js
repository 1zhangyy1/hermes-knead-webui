...
async function newSession(flash, options={}){
  ...
  const productDefaults=typeof currentAssistantProductSessionDefaults==='function'
    ? currentAssistantProductSessionDefaults()
    : {};
  const productWs=options&&options.workspace
    ? options.workspace
    : (productDefaults.workspace || (typeof currentAssistantWorkspacePath==='function' ? currentAssistantWorkspacePath() : ''));
  const inheritWs=productWs||switchWs||(S.session?S.session.workspace:null)||(S._profileDefaultWorkspace||null);
  ...
  const productId=(options&&options.productId)||productDefaults.productId||'';
  const productScope=(options&&options.productScope)||(productId?'product_usage':'');
  if(productId) reqBody.product_id=productId;
  ...
  const productToolsets = Array.isArray(options&&options.enabledToolsets)
    ...
    : (Array.isArray(productDefaults.enabledToolsets)
      ? productDefaults.enabledToolsets
      : (typeof currentAssistantProductToolsets === 'function' ? currentAssistantProductToolsets() : []));
  if(productToolsets&&productToolsets.length) reqBody.enabled_toolsets=productToolsets;
  ...
  const nextAiMode=!!(document.body&&document.body.dataset&&document.body.dataset.nextAiView);
  if(!nextAiMode&&_activeProject&&_activeProject!==NO_PROJECT_FILTER) reqBody.project_id=_activeProject;
  const data=await api('/api/session/new',{method:'POST',body:JSON.stringify(reqBody)});
  S.session=data.session;S.messages=data.session.messages||[];
  if(productId&&S.session){
    S.session.product_id=productId;
...
async function loadSession(sid){
  ...
  try {
    ...
    if(_msgInner){
      if(e.status===404){
        ...
        if (typeof openAssistantLanding === 'function') {
          openAssistantLanding();
        } else {
          _msgInner.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:14px;padding:40px;text-align:center;">开始一个新任务。</div>';
        }
        return;
      } else {
  ...
  if (typeof syncAssistantForLoadedSession === 'function') syncAssistantForLoadedSession(S.session);
  ...
  if(!INFLIGHT[sid]&&activeStreamId&&typeof loadInflightState==='function'){
    ...
    if(stored){
      INFLIGHT[sid]={
        ...
        productUiStatusContext:stored.productUiStatusContext||null,
        reattach:true,
      };
    }
  }
...
function _applySessionListPayload(sessData, projData){
  ...
  if (typeof hydrateAssistantDisplayTitlesForSessions === 'function') {
    hydrateAssistantDisplayTitlesForSessions(_allSessions);
  }
}
...
function renderSessionListFromCache(){
  ...
  // filter entirely.
  const profileFiltered=assistantFiltered;
  ...
  const nextAiTaskListMode=!!(document.body&&document.body.dataset&&document.body.dataset.nextAiView);
  const effectiveActiveProject=nextAiTaskListMode?null:_activeProject;
  const projectFiltered=
    effectiveActiveProject===NO_PROJECT_FILTER
      ?profileFiltered.filter(s=>!s.project_id)
      :(effectiveActiveProject?profileFiltered.filter(s=>s.project_id===effectiveActiveProject):profileFiltered);
  ...
  if(typeof hydrateAssistantDisplayTitlesForSessions==='function') hydrateAssistantDisplayTitlesForSessions(sessions);
  ...
  const assistantTitleForList=window._currentAiAssistantTitle||'当前 AI 产品';
  if(!nextAiTaskListMode&&!effectiveActiveProject&&sessions.length>0){
    const summary=document.createElement('div');
    summary.className='assistant-task-list-summary';
    const summaryTitle=document.createElement('div');
    summaryTitle.className='assistant-task-list-summary-title';
    summaryTitle.textContent=`「${assistantTitleForList}」处理过的任务`;
    const summaryMeta=document.createElement('div');
    summaryMeta.className='assistant-task-list-summary-meta';
    const taskCount=sessions.length;
    summaryMeta.textContent=`${taskCount} 个任务`;
  ...
  if(!nextAiTaskListMode&&(_allProjects.length>0||hasUnprojected)){
    const bar=document.createElement('div');
  ...
  if(effectiveActiveProject&&sessions.length===0){
  ...
  function _renderOneSession(s, isPinnedGroup=false){
    ...
    const rawTitle=(typeof assistantDisplayTitleForSession==='function')
      ? assistantDisplayTitleForSession(s)
