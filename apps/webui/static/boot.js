...
function applyBotName(){
  ...
  // Falls back to the product shell name.
  let name;
...
(async()=>{
  ...
  const _onboardingLoad = (typeof loadOnboardingWizard === 'function')
    ? Promise.race([
  ...
  if (urlSession) {
    // Direct task URLs must restore the task immediately. Onboarding is a
    // peripheral overlay and can finish in the background if its endpoint is slow.
    _onboardingLoad.catch(() => false);
  } else {
    await _onboardingLoad;
  }
  await renderSessionList();
  _initResizePanels();
  ...
  if(saved){
    try{
      ...
      const _bootRestoredSid=saved;
      await loadSession(_bootRestoredSid);
      ...
      const _stillViewingBootRestoredSession=S.session&&S.session.session_id===_bootRestoredSid;
      const _firstSendStarting=typeof _sendInProgress!=='undefined'&&_sendInProgress;
      if(_stillViewingBootRestoredSession && (S.session.message_count||0) === 0 && !_restoredInFlight && !_firstSendStarting){
