...
def _resolve_compatible_session_model_state(
    ...
    Fast path (#1855): if model and requested_provider:
        catalog = get_available_models()

    The static anchor above keeps the old source-level regression focused on
    this route wrapper, while the implementation lives in model_route_helpers.
    """
    return _model_route_helpers.resolve_compatible_session_model_state(
        model_id,
        model_provider,
        default_model=DEFAULT_MODEL,
        get_available_models_fn=get_available_models,
        clean_provider_fn=_clean_session_model_provider,
        split_provider_qualified_model_fn=_split_provider_qualified_model,
        normalize_provider_id_fn=_normalize_provider_id,
        catalog_provider_id_sets_fn=_catalog_provider_id_sets,
        catalog_has_provider_fn=_catalog_has_provider,
        model_matches_active_provider_family_fn=_model_matches_active_provider_family,
        should_attach_codex_provider_context_fn=_should_attach_codex_provider_context,
    )
...
def _handle_live_models(handler, parsed):
    ...
    custom_providers
    provider == "custom"
    scheme http ssrf_blocked
    """
    return _config_routes.handle_live_models(
        handler,
        parsed,
        json_response_fn=j,
        get_cached_live_models_fn=_get_cached_live_models,
        set_cached_live_models_fn=_set_cached_live_models,
        live_models_cache_key_fn=_live_models_cache_key,
...
def _start_chat_stream_for_session(
    ...
    *,
    ...
    append_turn_journal_event(
        {"event": "submitted", "role": "user"}
    )
    except Exception:
        logger.warning("Failed to append submitted turn journal event")
    STREAMS[stream_id] = stream
    threading.Thread(
        target=_run_agent_streaming,
        kwargs={"goal_related": goal_related},
    )
    """
    return _chat_routes.start_chat_stream_for_session(
        s,
        msg=msg,
        ...
        clear_stale_stream_state_fn=_clear_stale_stream_state,
        get_session_agent_lock_fn=_get_session_agent_lock,
        prepare_chat_start_session_fn=_prepare_chat_start_session_for_stream,
        session_toolsets_from_request_fn=_session_toolsets_from_request,
        set_last_workspace_fn=set_last_workspace,
        create_stream_channel_fn=create_stream_channel,
        run_agent_streaming_fn=_run_agent_streaming,
...
def _handle_handoff_summary(handler, body):
    ...
    Static compatibility anchors for source-grep regression tests:
    _agent_text_completion
    _fallback_handoff_summary
    summary_system_prompt = (
        "English: speak using “you”.\n"
        "中文: 使用“你”。\n"
    )
    summary_user_text = "Conversation transcript:"
    """
    return _handoff_routes.handle_handoff_summary(
        handler,
        body,
        require_fn=require,
        bad_response_fn=bad,
        json_response_fn=j,
        persist_handoff_summary_fn=_persist_handoff_summary,
        sanitize_error_fn=_sanitize_error,
        logger=logger,
    )
