...
def _signature_block() -> str:
    ...
    sig_end = STREAMING_AGENT_CACHE_PY.index("return hashlib.sha256", sig_start)
    return STREAMING_AGENT_CACHE_PY[sig_start:sig_end]
...
def test_profile_home_resolved_before_cache_signature():
    startup_py = (REPO / "api" / "streaming_worker_startup.py").read_text(encoding="utf-8")
    setup_py = (REPO / "api" / "streaming_agent_turn_setup.py").read_text(encoding="utf-8")
    profile_runtime_resolve = startup_py.index("activation = activate_streaming_profile_runtime_fn(")
    agent_setup_start = STREAMING_PY.index("_prepare_streaming_agent_turn_setup(")
    agent_setup_block = STREAMING_PY[agent_setup_start:agent_setup_start + 900]
    agent_for_turn_start = setup_py.index("get_and_register_agent_for_turn_fn(")
    agent_for_turn_block = setup_py[agent_for_turn_start:agent_for_turn_start + 900]
    startup_call = STREAMING_PY.index("_startup = _prepare_streaming_worker_startup(")
    assert startup_call < agent_setup_start
    assert profile_runtime_resolve != -1
    assert "profile_home=_profile_home" in agent_setup_block
    assert "profile_home=profile_home" in agent_for_turn_block
