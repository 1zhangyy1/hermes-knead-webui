...
def test_streaming_persists_reasoning_in_session():
    ...
    assert "self.reasoning_text = ''" in bridge_src, \
        "StreamingOutputBridge does not initialise reasoning_text"

    assert 'self.reasoning_text += text' in bridge_src, \
...
def test_streaming_restores_prior_reasoning_metadata_after_followup():
    ...
    assert "session.messages = merge_display_messages_after_agent_result(" in context_src, \
        "streaming_context.py must merge restored result messages into the visible transcript"
